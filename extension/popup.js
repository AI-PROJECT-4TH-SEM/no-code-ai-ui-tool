...
function setupGlobalActions() {
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    if (msg.type === "LAYOUT_APPLIED" || msg.type === "INSPECTOR_CLOSED") {
...
function updateDownloadBadge() {
  ...
  const total    = fixCount + (hasTheme ? 1 : 0)
  ...
  if (total > 0) btn.classList.add("ga-dl-ready")
  else btn.classList.remove("ga-dl-ready")
}
...
async function downloadAllChanges() {
  ...
  try {
    ...
    // ── Collect all applied fixes for the changelog ───────────────────────────
    const appliedFixCards = [...document.querySelectorAll(".btn-fix-inline.applied-inline")]
    const fixCount   = appliedFixCards.length
    const activeTheme = THEMES.find(t => t.id === activeThemeId)

    // ── Build changelog lines ─────────────────────────────────────────────────
    const changeLog = []
    if (fixCount > 0) {
      changeLog.push("  ACCESSIBILITY FIXES (" + fixCount + "):")
      appliedFixCards.forEach((btn, i) => {
        const title = btn.closest(".card")?.querySelector(".card-title")?.textContent || "Fix " + (i+1)
        changeLog.push("    " + (i+1) + ". " + title.trim())
      })
    }
    if (activeTheme) {
      changeLog.push("  THEME APPLIED:")
      changeLog.push("    " + activeTheme.name)
      changeLog.push("    CSS injected as <style id='cksa-theme'> inside <head>")
    }

    // Check for layout inspector changes (elements with style attributes added by inspector)
    const styledEls = [...document.querySelectorAll("[data-cksa-layout]")]
    if (styledEls.length > 0) {
      changeLog.push("  LAYOUT CHANGES (" + styledEls.length + " element(s)):")
      styledEls.forEach((el, i) => {
        changeLog.push("    " + (i+1) + ". <" + el.tagName.toLowerCase() + "> — " + el.getAttribute("data-cksa-layout"))
      })
    }

    // ── Inject theme CSS permanently ──────────────────────────────────────────
    if (activeTheme) {
      const styleTag = "<style id='cksa-theme'>/* ========== Chai Ke Sath AI Theme: " + activeTheme.name + " ========== */" + activeTheme.css + "/* ========== End Theme ========== */</style>" 
      finalHtml = finalHtml.includes("</head>")
     ? finalHtml.replace("</head>", styleTag + "</head>"): styleTag + "" + finalHtml}

    // ── Comment header with full changelog ────────────────────────────────────
    const separator = "  " + "=".repeat(55)
    const header = [
      "<!--",
      separator,
      "  CHAI KE SATH AI — FINAL MODIFIED PAGE",
      separator,
      "  Source URL : " + currentUrl,
      "  Downloaded : " + now.toLocaleString(),
      "  Score      : " + score + "/100",
      "  Total Changes: " + (fixCount + (activeTheme ? 1 : 0)),
      separator,
      "  WHAT WAS CHANGED:",
      ...(changeLog.length ? changeLog : ["  (no changes recorded)"]),
      separator,
      "-->",
      ""
    ].join("")

    finalHtml = header + finalHtml

    // ── Download ──────────────────────────────────────────────────────────────
    const blob     = new Blob([finalHtml], { type: "text/html;charset=utf-8" })
    const blobUrl  = URL.createObjectURL(blob)
    const safeName = (domain || "page").replace(/[^a-z0-9]/gi, "-").toLowerCase()
    const filename = safeName + "-final-"
      + now.getFullYear()
      + String(now.getMonth()+1).padStart(2,"0")
      + String(now.getDate()).padStart(2,"0") + ".html"
    ...
    btn.innerHTML = '<span class="ga-icon">✅</span><span class="ga-label">Saved!</span>'
    showToast("✅ Downloaded: " + filename, "success")
    setTimeout(() => { btn.innerHTML = origHTML; btn.disabled = false; updateDownloadBadge() }, 3500)
...
async function applyTheme(theme) {
  try {
    ...
    setTimeout(() => showToast(`${savedIcon} ${savedTo}`, "info"), 1800)
    updateDownloadBadge()
